...
import com.wspcgir.decisionbot.destinations.ChoiceListPageDestination
import com.wspcgir.decisionbot.destinations.EditRequirementPageDestination
...
import com.wspcgir.decisionbot.repository.entity.RequirementBox
...
fun MakeEditChoicePage(
    ...
    val answerList = remember { mutableStateOf(emptyList<ListItem<Answer, AnswerFieldContext>>()) }
    ...
    LaunchedEffect(answersMut.value) {
    ...
    LaunchedEffect(answersMut.value) {
        answerList.value = makeListItems(
            answersMut,
            editContext = {
                AnswerFieldContext(
            ...
            { item -> scope.launch { repo.deleteAnswer(item) } }
        )
    }
    ...
    EditChoicePage(object : EditChoicePageViewModel() {
        ...
        override fun updateAnswer(value: MutableState<Answer>, newDescription: String) {
            ...
            answersMut.value = answersMut.value.map {
                if (it.id == newAnswer.id) newAnswer else it
            }
        }
